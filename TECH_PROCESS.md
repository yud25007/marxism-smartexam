...
*   **背景**：国内首屏加载极慢（LCP > 10s）。
*   **优化策略**：
    1.  **分包剥离**：将 436KB 题目数据 `cloud_data.ts` 剥离出主包。
    2.  **CDN 镜像**：Google Fonts 和 KaTeX 切换至国内镜像（loli.net/baomitu）。
    3.  **SQL 特权化**：仅管理员触发云端查询，普通用户完全走静态包，消除 SQL 握手延迟。
    4.  **非阻塞加载**：Tailwind `defer` 加载，CSS 非阻塞引入。
*   **最终结果**：
    1.  **性能提升**：主包体积减小 40%，Gzip 后核心逻辑仅 109KB。
    2.  **遭遇挑战**：Zeabur 基础设施（Pod Sandbox）网络故障导致部署中断；国内 CDN 节点波动引发局部 404；带宽限速导致感知速度提升有限。
*   **处理决策**：为保证系统 100% 稳定性，根据用户要求执行了 `git reset --hard` 全量回退，退回至 `f592cce`。
*   **保留产物**：保存了详细的《性能优化方案与执行总结报告》(PERF_OPTIMIZATION_REPORT.md) 供后续架构演进参考。
